...
export default function ListingDetailPage() {
  ...
  if (!listing) return <div className="container-page py-20 text-center text-muted-foreground">Listing not found</div>;
  ...
  return (
    <div className="container-page py-8 space-y-12 animate-fade-up">
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="space-y-3">
          <div className="aspect-square rounded-lg overflow-hidden bg-muted">
            <img src={listing.imageUrls?.[0] || "/placeholder.svg"} alt={listing.title} className="w-full h-full object-cover" />
          </div>
          {listing.imageUrls?.length > 1 && (
            <div className="flex gap-2 overflow-x-auto">
              {listing.imageUrls.slice(1).map((url, i) => (
                <div key={i} className="w-20 h-20 rounded-md overflow-hidden bg-muted shrink-0">
        ...
        <div className="space-y-6">
          ...
          <p className="text-3xl font-bold">{listing.currency} {listing.price.toFixed(2)}</p>

          <p className="text-muted-foreground leading-relaxed">{listing.description}</p>

          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            <span>{listing.stockQuantity > 0 ? `${listing.stockQuantity} in stock` : "Out of stock"}</span>
            {listing.country && <span>· Ships from {listing.country}</span>}
          </div>
          ...
          {user && listing.stockQuantity > 0 && (
            <div className="flex items-center gap-3">
              <div className="flex items-center border rounded-lg">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="p-2 hover:bg-muted transition-colors active:scale-95"><Minus className="w-4 h-4" /></button>
                <span className="px-3 text-sm font-medium tabular-nums">{qty}</span>
                <button onClick={() => setQty(Math.min(listing.stockQuantity, qty + 1))} className="p-2 hover:bg-muted transition-colors active:scale-95"><Plus className="w-4 h-4" /></button>
              </div>
      ...
      <section className="space-y-6 border-t pt-8">
        ...
        {reviews.length === 0 ? (
          ...
          <div className="space-y-4">
            {reviews.map((r) => (
              <div key={r.id} className="p-4 rounded-lg bg-card border space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-xs font-bold">
                      {r.userDisplayName?.charAt(0)?.toUpperCase() || "?"}
